...
export class Tree {
      ...
      while (nodes.length != 0) {
        ...
        while (children.length != 0) {
          ...
          if (!nodeActual.getState()) {
            ...
            } else this.searchPlays(nodeActual, 0, player);
  ...
  public searchPlays(pNode: Nodo, pNum: number, pPlayer: number) {
    ...
    for (let index = 0; index < positions.length; index++) {
      let newPosition = this.positionFollow(
        positions[index],
        pNum,
        board,
        pPlayer
      ...
      if (newPosition[0] != -1) {
        ...
        if (newBoard[0][5] == 7 || newBoard[2][5] == 7) {
          ...
          pNode.addChidren(
            new Nodo(newBoard, false, this.isSecondTurn(newPosition, pPlayer))
          );
        }
      }
    }
    ...
    for (let index = 0; index < positions.length; index++) {
      ...
      if (newPosition[0] != -1) {
        ...
        if (newBoard[0][5] == 7 || newBoard[2][5] == 7) {
          ...
          pNode.addChidren(
            new Nodo(newBoard, false, this.isSecondTurn(newPosition, player))
          );
        }
  ...
  private isSecondTurn(pNewPosition: number[], pPlayer: number): boolean {
    ...
    if (
      pNewPosition[0] == nivel &&
      (pNewPosition[1] == 0 || pNewPosition[1] == 6)
    ) {
      return true;
  ...
  private getPositions(pBoard: number[][], pPlayer: number): number[][] {
    ...
    if (pBoard[nivel][4] > 0) positions.push([nivel, 4]);
    return positions;
  }
  ...
  private getNewBoard(
    ...
    if (pNewPosition[0] == nivel && pNewPosition[1] == 5) {
      ...
      if (pPosition[0] == nivel && pPosition[1] == 4) pBoard[nivel][4] -= 1;
      else pBoard[pPosition[0]][pPosition[1]] = 0;
    }
    return pBoard;
  }

  private positionFollow(
    pPosition: number[],
    pNum: number,
    pBoard: number[][],
    player: number
    ...
    if (
      pos[0] != -1 &&
      pBoard[pos[0]][pos[1]] == player &&
      (pos[1] < 4 || pos[1] > 5 || pos[0] == 1)
    ) {
      pos = [-1, -1];
    } else if (pos[0] == 1 && pos[1] == 3 && pBoard[pos[0]][pos[1]] != 0) {
      pos = [-1, -1];
    }
    return pos;
  }
  ...
  private minimax(
    ...
    if (pMaximizingPlayer) {
      ...
      for (let index = 0; index < pNode.getChildren().length; index++) {
        ...
        if (pNode.getAlpha() >= pNode.getBeta()) break;
      }
